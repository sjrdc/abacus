    ...
    namespace detail
    {
        ...
        namespace grammar
        {
            ...
            constexpr auto make_variable =
                [](auto& context)
                {
                    auto& store = x3::get<variable_store_tag>(context).get();
                    x3::_val(context) = store.get(x3::_attr(context));
                };
        ...
        ast::operand parse(const std::string& expression, variable_store& store)
        {
            auto f = begin(expression), l = end(expression);
            ast::operand out;
            auto parser = x3::with<variable_store_tag>(std::ref(store))[grammar::expression_rule];
            if (phrase_parse(f, l, parser, x3::space, out))
            {
                if (f != l)
                {
            ...
            throw std::runtime_error("unknown error occurred on parsing " + expression);
        }
    
        ast::operand parse(const std::string& expression)
        {
            auto store = variable_store();
            return parse(expression, store);
        }
    }
}
